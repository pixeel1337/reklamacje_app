...
from config import STATUSY_OTWARTE
...
def odswiezWszystko(
    ...
    for r in dane:
        ...
        if tylko_otwarte and status not in STATUSY_OTWARTE:
            continue
        ...
        odpowiedzialny = r.get("odpowiedzialny") or r.get(
            "osoba_rejestrujaca", ""
        )
        opis = r.get("opis", "")
...
def zmienStatusZaznaczonego(tree, nowy_status, odswiez_callback):
    ...
    wynik = aktualizujReklamacje(
        nr_rek, {"status": nowy_status}, regeneruj_pliki=True
    )
    if wynik:
        messagebox.showinfo(
        ...
        messagebox.showerror(
            "Błąd", f"Nie udało się zmienić statusu dla {nr_rek}."
        )
